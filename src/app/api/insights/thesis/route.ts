...
export async function GET() {
  try {
    ...
    const thesisFilePath = `outputs/${latestFolder}/thesis.json`;
    ...
    if (!getObjectResponse.Body) {
      return NextResponse.json({ error: 'thesis.json not found in latest folder' }, { status: 404 });
    }
    ...
    console.error('Error fetching thesis.json:', error);
