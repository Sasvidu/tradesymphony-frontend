...
export async function GET() {
  try {
    ...
    const folders = listObjectsResponse.CommonPrefixes.map((prefix) =>
      prefix.Prefix!.replace("outputs/", "").replace("/", "")
    );
    ...
    if (!getObjectResponse.Body) {
      return NextResponse.json(
        { error: "investment.json not found in latest folder" },
        { status: 404 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { error: "Error parsing investment.json" },
        { status: 500 }
      );
    }
  } catch (error) {
